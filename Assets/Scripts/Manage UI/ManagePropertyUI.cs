...
public class ManagePropertyUI : MonoBehaviour
{
    ...
    public void SetProperty(List<MonopolyNode> nodes, Player owner)
    {
        ...
        buyHouseButtonText.text = ("ПОСТРОИТЬ <color=red>-" + owner.GetHouseCostForPlayer(nodesInSet[0]) + "BYN");//DESIGN 
        sellHouseButtonText.text = ("ПРОДАТЬ <color=green>+" + owner.GetSellHouseCostForPlayer(nodesInSet[0]) + "BYN");//DESIGN 
